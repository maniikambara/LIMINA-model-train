...
sepenuhnya milik dan dikelola di luar proyek ini (lihat sql/schema.sql).

Ini jalur BACA yang dipakai 01_ambil_data.ipynb untuk mengisi cache lokal
`data/raw/` yang kemudian dibaca eda_and_feature_engineering.ipynb --
berbeda dari sectors_fetcher/storage/ (belum ada di checkout ini) yang
jadi jalur TULIS dipakai sectors_fetcher/main.py.

Kredensial dibaca HANYA dari sectors_fetcher.config (yang HANYA membaca
dari SUPABASE_URL/SUPABASE_KEY) -- tidak ada kredensial tertulis di
berkas ini, karena berkas ini bisa masuk repositori publik.
...
bisa memeriksa None dan berhenti dengan pesan jelas (01_ambil_data.ipynb),
...
def get_client(url: str | None = None, key: str | None = None) -> "Client | None":
    ...
    config.SUPABASE_URL/SUPABASE_KEY (hasil baca variabel lingkungan)
    kalau keduanya tidak diisi eksplisit.
...
def normalisasi_tabel_suspensi(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sectors_fetcher/config.py (KOLOM_TANGGAL_SUSPENSI, KOLOM_ALASAN_SUSPENSI)
    -- fungsi ini membacanya, bukan menebak lewat kode di dalam notebook.
    ...
    KeyError membingungkan saat eda_and_feature_engineering.ipynb jalan.
    """
    df = df.copy()
    ...
    nama_tabel = config.SUPABASE_TABLES["suspensions"]
    kolom_wajib = {"symbol", "event_date", "reason"}
    hilang = kolom_wajib - set(df.columns)
    if hilang:
        raise RuntimeError(
            f"Tabel {nama_tabel} tidak punya kolom {hilang} setelah "
            ...
            f"nilai itu di sectors_fetcher/config.py dengan nama kolom sungguhan di "
            f"tabel Supabase Anda, lalu jalankan ulang 01_ambil_data.ipynb."
        )
    return df
...
KOLOM_WAJIB_TABEL = {
    ...
    # daily_full_universe_close, per AMBANG-panduan-api-sectors.md, HANYA
    # menyediakan close -- volume/market_cap sering kosong di sini, jangan
    # diwajibkan.
...
def validasi_kolom_tabel(df: pd.DataFrame, nama_tabel: str) -> None:
    ...
    modul lain (KOLOM_WAJIB_TABEL). Dipanggil 01_ambil_data.ipynb untuk
    kelima tabel selain stock_suspensions (yang punya jalur sendiri,
    normalisasi_tabel_suspensi, karena nama kolomnya bisa disesuaikan
    lewat sectors_fetcher/config.py).
    ...
    tidak jelas di eda_and_feature_engineering.ipynb. Nama tabel tak
    dikenal dilewati, bukan dianggap salah.
    """
    ...
    if hilang:
        raise RuntimeError(
            ...
            f"KOLOM_WAJIB_TABEL di sectors_fetcher/supabase_io.py."
        )
